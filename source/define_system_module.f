...
c======== tree parameters========================================================
        ...
        elseif(findstring('torder',directive,idum))then

c     order of approximation for treecode

          torder=intstr(directive,lenrec,idum)
          if(idnode.eq.0)write(nrite,
     x      "(/,1x,'order of taylor approximation for
     x         treecode',3x,i10)")torder

        elseif(findstring('parnode',directive,idum))then

c     maximum number of particles in a leaf

          maxparnode=intstr(directive,lenrec,idum)
          if(idnode.eq.0)write(nrite,
     x      "(/,1x,'maximum number of particles in a 
     x      leaf',3x,i10)")maxparnode

        elseif(findstring('mactheta',directive,idum))then
          
c     specify allowed density variation
          
          mactheta=dblstr(directive,lenrec,idum)
          if(idnode.eq.0)write(nrite,
     x      "(/,1x,'multipole acceptance criterion for 
     x        tree   ',3x,1p,e12.4)")mactheta

        elseif(findstring('lattlim',directive,idum))then

c     range of periodic images

          lattlim=intstr(directive,lenrec,idum)
          if(idnode.eq.0)write(nrite,
     x      "(/,1x,'range of periodic images -
     x        treecode',3x,i10)")lattlim

c     type of treecode

        elseif(findstring('taylor',directive,idum))then
          if(idnode.eq.0)write(nrite,
     x      "(/,1x,'Using a Taylor Polynomial -
     x        treecode')")

        elseif(findstring('tricubic',directive,idum))then
          if(idnode.eq.0)write(nrite,
     x      "(/,1x,'Using a Tricubic Interpolation -
     x        treecode')")

c        elseif(findstring('marsden',directive,idum))then
c          if(idnode.eq.0)write(nrite,
c     x      "(/,1x,'Using the Lekien, Marsden Tricubic
c     x    formulation')")
...
c=========logical for treecode==========================================
        elseif(findstring('tree',directive,idum))then

          keyfce=16
          if(idnode.eq.0)write(nrite,
     x      "(/,/,1x,'Electrostatics : Treecode approximation')")
